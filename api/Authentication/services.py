...
def send_sms(number,otp):
    ...
    if phone_number_exists and len(number) == 10:
        response = requests.get("https://sms.arkesel.com/sms/api?action=send-sms&api_key=OlZNNGJVWldVVE5UdEl2eEs=&to="+ number +"&from=COMPSSA-HTU&sms="+"DO NOT SHARE!!, \nYour OTP is "+ otp +"\nPlease Enter it in 15 minutes.\nremember not to share with anyone !!!")
        print(response.json)
